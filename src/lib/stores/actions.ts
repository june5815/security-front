import type {BaseStore, PaginationState, ListStore, PaginatedStore} from "@/lib/stores/types.ts";
import type {PaginationParams, PageResponse} from "@/lib/types";
...
export function createPaginatedStoreActions<T, P extends PaginationParams>(
    {set, get, fetchApi, initialData, keyExtractor = e => (e as any)['id'] as string} : {
      set: SetType<PaginatedStore<T, P>>,
      get: () => PaginatedStore<T, P>,
      fetchApi: (p: P) => Promise<PageResponse<T>>,
      initialData?: Partial<{
        data: T[];
        params: P;
  ...
  const _initialData = {data: [], params: {limit: 20, page: 1} as P, paginationState: {hasNext: false, totalCount: 0} as PaginationState, ...initialData};
  return ({
    data: _initialData.data,
    add: newItem => {
      set(state => {
        ...
        const cursorState: PaginationState = {...state.paginationState, totalCount: state.paginationState.totalCount+1}
        ...
        // 정렬 기준이 없으면 맨 앞에 추가
        if (!sortBy || !sortDirection) {
          return {data: [newItem, ...data], paginationState: cursorState};
        }
        ...
        return {data: sortedData, paginationState: cursorState};
      })
    },
    update: (id, newData) => {
      set((state) => ({
    ...
    delete: (id) => {
      set((state) => ({
        data: state.data.filter((item) => keyExtractor(item) !== id),
        paginationState: {...state.paginationState, totalCount: state.paginationState.totalCount-1}
      }))
    },
    count: () => get().paginationState.totalCount,
    ...
    paginationState: _initialData.paginationState,
    hasNext: () => get().paginationState.hasNext,
    ...
    fetchMore: async (options) => {
      const {loading, paginationState} = get();
      if (loading && options?.ignoreLoading !== true) {
        console.warn(`로딩 중이므로 요청이 무시되었습니다.`);
        return;
      }
      ...
      try {
        ...
        const result = await fetchApi({...params, page: paginationState.nextPage} as P);
