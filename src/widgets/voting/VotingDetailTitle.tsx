...
export default function VotingDetailTitle({ data }: VotingDetailTitleProps) {
  ...
  const handleEdit = () => {
    router.push(`/admin/voting-edit/${data.id}`);
  };
  const handleDelete = () => {
    alert('투표 삭제');
  };
  ...
  const statusTextMap: Record<PollDetailDto['status'], string> = {
    PENDING: '투표전',
    IN_PROGRESS: '투표중',
    CLOSED: '마감',
  };
    ...
    <div>
      ...
      <div className='mt-[30px] flex w-full items-center justify-between border-b border-gray-100 pb-[16px] text-[14px]'>
        <div className='flex items-center gap-[12px]'>
          <span className={cn('text-main', data.status === 'CLOSED' && 'text-gray-400')}>
            {statusTextMap[data.status]}
          </span>
          <span className='text-gray-200'>|</span>
          <span className='text-gray-500'>투표권자</span>
          <span className='text-gray-400'>{getBuildingPermission(data.building || 0)}</span>
        </div>
        {isAdminPage && (
