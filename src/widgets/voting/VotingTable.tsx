...
export default function VotingTable({ data, currentPage, itemsPerPage, loading }: Props) {
            ...
            data.map((item, index) => (
              <tr key={item.id}>
                <td className={tdClass}>
                  {index+1+((itemsPerPage || 0) * ((currentPage || 1) -1))}
                </td>
                <td className={`${tdClass} hover:text-main`}>
                  <Link
                    href={
                      isAdminPage
                        ? `/admin/voting/detail/${item.id}`
                        : `/resident/voting/detail/${item.id}`
                    }
                  >
                ...
                <td className={tdClass}>{item.author.name}</td>
                <td className={tdClass}>{getBuildingPermission(item.building || 0)}</td>
                ...
                {isAdminPage && (
                  <td className={tdClass}>
                    <div className='flex items-center justify-center gap-4'>
                      {item.author.id === useAuthStore.getState().authentication?.id && (
                        <button
                      ...
                      <button
                        onClick={() => handleDelete(item.id, item.startDate, item.endDate)}
                        className='cursor-pointer'
                      >
