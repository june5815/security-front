...
import { importResidentsFromFile } from '@/lib/api/residents';
...
export default function AddFileModal({
  ...
  const handleRegisterClick = async () => {
    ...
    try {
      const response = await importResidentsFromFile(selectedFile)

      alert(`파일이 성공적으로 업로드되었습니다. (${response.count || 0}건)`);
      await fetchData();
      handleModalClose();
    } catch (error) {
      console.error(error);
