...
export default function RegisterSingleModal({
  ...
  const initialFormState: ResidentCreateRequest = {
    apartmentId: authentication?.adminOf?.id || '',
    building: apartment?.buildings[0] || -1,
    unit: apartment?.units[0] || -1,
    name: '',
    contact: '',
    email: '',
    isHouseholder: true,
  ...
  const [formData, setFormData] = useState<ResidentCreateRequest>(initialFormState);

  useEffect(() => {
    if (authentication?.adminOf?.id) {
      const apartmentId = authentication?.adminOf?.id;
      updateApartmentParams({ id: apartmentId });
      setFormData((prev) => ({
        ...prev,
        apartmentId,
      }));
  ...
  const isFormComplete = INPUT_FIELDS.every((f) => String(formData[f.key]).trim() !== '');
