...
export const adminSignupSchema = z
  .object({
    ...
    apartmentOfficeNumber: z.string().min(1, '관리소 번호를 입력해주세요'),

    apartmentDescription: z.string().min(1, '아파트 소개를 입력해주세요'),

    apartmentBuildingNumberFrom: numberFieldSchema('동'),

    apartmentBuildingNumberTo: numberFieldSchema('동'),

    apartmentFloorCountPerBuilding: numberFieldSchema('층'),

    apartmentUnitCountPerFloor: numberFieldSchema('호'),
  })
...
//입주민 회원가입 스키마
export const residentSignupSchema = () =>
  z
    .object({
      username: z
        .string()
      ...
      email: z
        ...
        .min(1, AUTH_VALIDATION.EMAIL.REQUIRED_MESSAGE)
        .email(AUTH_VALIDATION.EMAIL.INVALID_MESSAGE),
      building: z.string().min(1, '아파트 동을 선택해주세요'),
      unit: z.string().min(1, '아파트 호수를 선택해주세요'),
    })
    .refine((data) => data.password === data.passwordConfirm, {
      path: ['passwordConfirm'],
      message: AUTH_VALIDATION.PASSWORD_CONFIRMATION.MISMATCH_MESSAGE,
